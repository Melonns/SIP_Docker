...
const DetailIntern = () => {
  ...
  const fetchLogbookChart = async () => {
    ...
    try {
      const res = await apiClient.get(`/mentor/interns/${id}/logbook-chart`);
  ...
  // Profile is loaded from GET /mentor/interns/:id; removed redundant /profile endpoint to avoid duplicate fetches
  // (previously implemented in fetchAdminProfile)
 

  // Attendance summary is loaded from GET /mentor/interns/:id; removed redundant helper to avoid duplicate fetches
  ...
  const fetchAttendanceDetail = async (page = 1) => {
    ...
    try {
      ...
      const res = await apiClient.get(`/mentor/interns/${id}/attendance-detail?${params_obj.toString()}`);
      const payload = res.data || {};
  ...
  const handleLogbookAction = (action) => {
    if (!selectedLogDetail || !selectedLogDetail.logbookId) {
      return;
    }
  ...
  const executeLogbookAction = async () => {
    setActionLoading(true);
    try {
      const statusValue = confirmAction === 'approve' ? 'verified' : 'revision_needed';
      await apiClient.post(`/mentor/logbook/${selectedLogDetail.logbookId}/verify`, {
        status_verifikasi: statusValue,
        feedback: feedbackInput || ''
      });

      setStatusMessage({
        title: confirmAction === 'approve' ? "Verified" : "Revision Needed",
        desc: confirmAction === 'approve' 
          ? "The logbook entry has been approved successfully."
          : "The logbook entry has been marked for revision."
      });
  ...
  const toggleSelectAllLogbooksOnPage = () => {
    if (selectableLogbookIdsOnPage.length === 0) return;
    setSelectedLogbookIds(prev => {
      const allSelected = selectableLogbookIdsOnPage.every(id => prev.includes(id));
      if (allSelected) return prev.filter(id => !selectableLogbookIdsOnPage.includes(id));
      return Array.from(new Set([...prev, ...selectableLogbookIdsOnPage]));
    });
  };
  ...
  const handleBulkApproveSelected = async () => {
    ...
    try {
      const results = await Promise.all(idsToApprove.map(async (logbookId) => {
        try {
          await apiClient.post(`/mentor/logbook/${logbookId}/verify`, {
            status_verifikasi: 'verified',
            feedback: ''
          });
          return { ok: true };
        } catch (err) {
          console.error('Bulk approve failed for logbook:', logbookId, err);
          return { ok: false };
        }
      }));
      ...
      if (successCount > 0) {
        ...
        setStatusMessage({
          title: 'Verified',
          desc: `${successCount} logbook${successCount > 1 ? 's' : ''} approved.${failCount ? ` ${failCount} failed.` : ''}`
        });
      } else {
        setStatusType('error');
        setStatusMessage({ title: 'Error', desc: 'Failed to approve selected logbooks.' });
      }
  ...
  const fetchDailySummary = async (page = 1, statusFilters = appliedStatus, query = undefined, startDate = appliedStartDate, endDate = appliedEndDate) => {
    ...
    try {
      ...
      const res = await apiClient.get(`/mentor/interns/${id}/daily-summary`, { params: paramsObj });
      const payload = res.data || {};

      // DEBUG: Log payload and params for troubleshooting pagination
      try {
        console.debug("[DetailIntern] fetchDailySummary - params:", paramsObj, "page:", page, "itemsPerPage:", itemsPerPage);
        console.debug("[DetailIntern] fetchDailySummary - payload keys:", Object.keys(payload), payload.data ? { daily_summary_length: (payload.data.daily_summary || []).length, pagination: payload.data.pagination } : { daily_summary_length: (payload.daily_summary || []).length, pagination: payload.pagination });
      } catch (e) {
        console.debug("[DetailIntern] fetchDailySummary - debug log failed", e);
      }
      ...
      let mapped = (dailySummary || []).map((item, idx) => {
        ...
        // Get logbook status with priority logic
        let logbookStatus = '-';
    ...
    apiClient
      ...
      .then((res) => {
        ...
        setIntern((prev) => ({
          ...
          id: user.id_mahasiswa || user.student_id || user.identifier || user.user_id || prev.id,
  ...
  return (
    <div className="pt-8 pb-8 pl-2 w-full bg-[#F8FAFC] min-h-screen font-sans text-slate-800 -mt-8">
            ...
            <table className="w-full text-left border-collapse min-w-0">
              <thead>
                <tr className="text-sm font-bold text-slate-900 border-b border-slate-100 bg-slate-50/30">
                  <th className="px-2 py-4 w-20 text-center">
                        ...
                        <td className="px-2 py-4 text-center">
                          {!isAbsentRow && (
                            <button
                              ...
                              className={`inline-flex items-center justify-center h-[34px] w-[34px] text-white rounded-lg transition-colors shadow-sm active:scale-95 group ${
                                r.status === 'Pending' || r.status === 'pending'
                                  ? 'bg-[#22C55E] hover:bg-[#16A34A] shadow-green-100'
                                  : 'bg-[#354C8F] hover:bg-[#2a3c70] shadow-indigo-100'
                              }`}
                              title={r.status === 'Pending' || r.status === 'pending' ? 'Process Action' : 'View Details'}
                            >
                              {r.status === 'Pending' || r.status === 'pending' ? (
          ...
          <div className="p-4 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
            ...
            <div className="flex items-center gap-4 order-1 md:order-2">
              ...
              {(() => {
                ...
                try { console.debug("[DetailIntern] Pagination render - pageCurrent:", pageCurrent, "pageTotal:", pageTotal); } catch(e){}
                const pageItems = getPageItems(pageCurrent, pageTotal);
                try { console.debug("[DetailIntern] Pagination render - pageItems:", pageItems); } catch(e){}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl relative max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  disabled={selectedLogDetail.status !== 'Pending'}
                ></textarea>
              </div>
            ...
            <div className="p-6 border-t border-slate-100 bg-white rounded-b-2xl z-10 flex gap-3 justify-end">
              ...
              {selectedLogDetail.status === 'Pending' || selectedLogDetail.status === 'pending' ? (
                <>
                  <button onClick={() => handleLogbookAction('revision_needed')} className={`${btnRejectClass} w-36`}>Revision</button>
                  <button onClick={() => handleLogbookAction('approve')} className={`${btnSuccessClass} w-36`}>Approve</button>
                </>
              ) : (
              ...
              {activeLogbookTab === 'needs-approval' ? (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setModalArchiveCategory(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 bg-white hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-[#354C8F]/20"
                  >
      ...
      {/* Confirmation Modal for Logbook Action */}
      {showConfirmModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 relative">
            <div className="text-center">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${confirmAction === 'revision_needed' ? 'bg-red-50' : 'bg-green-50'}`}>
                {confirmAction === 'revision_needed' ? <AlertCircle className="text-red-500" size={40} strokeWidth={2} /> : <AlertCircle className="text-green-500" size={40} strokeWidth={2} />}
              </div>
              <h3 className="text-[20px] font-bold text-[#27345A] mb-2">
                {confirmAction === 'approve' ? 'Approve Logbook?' : 'Request Revision?'}
              </h3>
              <p className="text-slate-500 text-sm mb-8">
                {confirmAction === 'approve' 
                  ? 'Approve this logbook entry and mark it as verified.'
                  : 'Request the intern to revise and resubmit this logbook entry.'}
              </p>
              <div className="flex gap-3">
                <button onClick={() => setShowConfirmModal(false)} disabled={actionLoading} className={`${btnSecondaryClass} w-full justify-center`}>Cancel</button>
                <button disabled={actionLoading} onClick={executeLogbookAction} className={`flex items-center justify-center gap-2 w-full py-3.5 px-6 rounded-xl font-bold text-sm shadow-md text-white transition-all active:scale-95 disabled:opacity-70 disabled:cursor-wait ${confirmAction === 'revision_needed' ? 'bg-[#EF4444] shadow-red-200 hover:bg-[#DC2626]' : 'bg-[#22C55E] shadow-green-200 hover:bg-[#16A34A]'}`}>
                  {actionLoading && <Loader2 size={16} className="animate-spin" />}
                  {actionLoading ? (confirmAction === 'approve' ? 'Approving...' : 'Requesting...') : (confirmAction === 'approve' ? 'Approve' : 'Revision')}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 relative">
            <div className="text-center">
              ...
              <h3 className="text-[20px] font-bold text-[#27345A] mb-2">{statusMessage.title}</h3>
              <p className="text-slate-500 text-sm mb-8">{statusMessage.desc}</p>
              <button onClick={() => setShowStatusModal(false)} className={`${btnSuccessClass} w-full justify-center`}>OK</button>
            </div>
          </div>
        </div>
      )}
