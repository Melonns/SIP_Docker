...
const InternMonitoring = () => {
    ...
    const fetchInterns = async (page = 1, overrides = {}) => {
        ...
        const statusVal = overrides.status !== undefined ? overrides.status : filterStatus; // 'All' | 'Active' | 'Inactive' | null
        ...
        setLoading(true);
        try {
            const params = { include_progress: 1, page, per_page: itemsPerPage, as_role: 'mentor' };
            ...
            const res = await apiClient.get('/mentor/interns', { params, signal: controller.signal });
            ...
            const mapped = list.map((i) => ({
                id: i.id_mahasiswa ?? i.user_id ?? i.id ?? i.user?.id,
            ...
            // Apply status filter (based on actual status field: active/inactive)
            let finalList = mapped;
            if (statusVal && statusVal !== 'All') {
                finalList = mapped.filter((item) => {
            ...
            setDisplayInterns(finalList);
            setCurrentPage(meta.current_page || page);
            setTotalPages(meta.last_page || Math.max(1, Math.ceil((finalList.length) / itemsPerPage)));
            setTotalEntries(finalList.length);
            setShowingFrom(finalList.length ? (page - 1) * itemsPerPage + 1 : 0);
            setShowingTo(finalList.length ? Math.min(page * itemsPerPage, finalList.length) : 0);
        } catch (err) {
    ...
    const goToDetail = (item) => {
        navigate(`/mentor/interns/${item.id}`, { state: { intern: item } });
    };
    ...
    */
    ...
    return (
        <div className="pt-8  pb-8 pl-2 w-full bg-slate-50 min-h-screen font-sans text-slate-800 -mt-9">
                            ...
                            <div>
                                ...
                                <div>
                                    <CustomSelect
                                        ...
                                        options={[{ value: 'All', label: 'All Universities' }, ...(availUni || []).map(u => { const v = (u && (u.nama_universitas || u.nama || u.universitas || u.name)) ? (u.nama_universitas || u.nama || u.universitas || u.name) : u; return { value: v, label: v }; })]}
